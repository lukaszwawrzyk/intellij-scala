  ...
  override def from(
    ...
    createOutputToCacheMap(context).map { outputToCacheMap =>
      ...
      CompilationData(sources, classpath, output,
        compilerConfiguration.scalacOptions ++ additionalOptions,
        compilerConfiguration.javacOptions,
        compilerConfiguration.order,
        cacheFile, relevantOutputToCacheMap, outputGroups,
  ...
  private def createOutputToCacheMap(context: CompileContext): Either[String, Map[File, File]] = {
    ...
    outputClashesIn(targetToOutput).toLeft {
      for ((target, output) <- targetToOutput.toMap)
        yield (output, new File(output, s"cache-${target.getPresentableName}.zip"))
    }
  }
